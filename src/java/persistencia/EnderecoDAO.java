...
public class EnderecoDAO {
    ...
    public static void inserirEndereco(EnderecoEntidade e) throws SQLException {
        String Query = "INSERT INTO endereco(id_usuario, logradouro, numero, complemento, bairro, cep, uf, cidade, coordenada) VALUES(?,?,?,?,?,?,?,?,0)";
        ...
        pst.setInt(7, e.getUf());
        pst.setInt(8, e.getCidade());
        pst.executeUpdate();

    }

    public static void editarEndereco(EnderecoEntidade end) throws SQLException {
        String Query = "UPDATE  endereco SET logradouro = ?, numero=?, complemento=?, bairro=?, cep=?, uf=?, cidade=?, coordenada=? where id_usuario = ?;";

        PreparedStatement pst = BaseDados.getInstancia().prepareStatement(Query);
        pst.setString(1, end.getLogradouro());
        pst.setInt(2, end.getNumero());
        pst.setString(3, end.getComplemento());
        pst.setString(4, end.getBairro());
        pst.setString(5, end.getCep());
        pst.setInt(6, end.getUf());
        pst.setInt(7, end.getCidade());
